...
import { _getMouseAbstractPoint, createCellRange, DrawGrid } from './core/DrawGrid';
...
export class ListGrid<T> extends DrawGrid implements ListGridAPI<T> {
  ...
  _getMouseRelativePoint(e: TouchEvent | MouseEvent): { x: number; y: number } | null {
    const abstractPos = _getMouseAbstractPoint(this,e)
    if (abstractPos) {
      return {
  ...
  setCellOverflowType(col: number, row: number, overflowType: false | string): void {
    const key = `${ col }:${ row }`
    if (overflowType) {
